...
class BatchNormalization : public Node {
	...
	virtual void print(std::ostream &dst) const override
	{
		...
		dst << "\t float epsilon = " << epsilon << std::endl;
		dst << "\t float momentum = " << momentum << std::endl;

		dst << "\t int32_t batch_size = " << batch_size << std::endl;
		dst << "\t int32_t num_chan = " << num_chan << std::endl;

		dst << "\t uint32_t height = " << input->data_dim[2] << std::endl;
		dst << "\t uint32_t width = " << input->data_dim[3] << std::endl;
